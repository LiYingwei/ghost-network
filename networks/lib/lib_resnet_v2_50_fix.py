...
def dropout_fix(x, keep_prob, noise_shape=None, seed=None, name=None):  # pylint: disable=invalid-name
    ...
    with ops.name_scope(name, "dropout", [x]) as name:
        ...
        with tf.variable_scope("fix_dropout", reuse=tf.AUTO_REUSE):
            ...
            random_result = tf.get_variable('random_{:d}'.format(np.random.randint(0, 65536)), shape=np_random.shape, initializer=tf.constant_initializer(np_random))
...
def bottleneck(inputs,
    ...
    with variable_scope.variable_scope(scope, 'bottleneck_v2', [inputs]) as sc:
        ...
        output = dropout_fix(output, keep_prob=FLAGS.keep_prob)
